...
# Create a new figure for q_size data
...
# Plot q_size_f with freqs as x-axis
ax5.plot(freqs, q_size_f['mean'], marker='o', label='Freq', color=color1)

# Fill the area around the line with a shadow representing errors
ax5.fill_between(freqs, 
                 [m - s for m, s in zip(q_size_f['mean'], q_size_f['std'])], 
                 [m + s for m, s in zip(q_size_f['mean'], q_size_f['std'])], 
...
# Plot q_size_n with nodes as x-axis
ax6.plot(nodes, q_size_n['mean'], marker='o', label='Nodes', color=color2)

# Fill the area around the line with a shadow representing errors
ax6.fill_between(nodes, 
                 [m - s for m, s in zip(q_size_n['mean'], q_size_n['std'])], 
                 [m + s for m, s in zip(q_size_n['mean'], q_size_n['std'])], 
                 color=color2, alpha=0.3)

# Set labels and title for the x-axis and y-axis
ax5.set_xlabel('Frequency - Nodes', fontsize=12)
ax5.set_ylabel('Mean Queue Size (Freq)', color=color1)
ax6.set_ylabel('Mean Queue Size (Nodes)', color=color2)
ax5.set_title('Mean Queue Size vs. Frequency and Nodes')
...
# Set x-axis label for the second plot (Time)
ax6.set_xlabel('Nodes', labelpad=10)

# Display the legend
lines4, labels4 = ax5.get_legend_handles_labels()
lines5, labels5 = ax6.get_legend_handles_labels()
ax5.legend(lines4 + lines5, labels4 + labels5, loc='upper left')
...
# Create a new figure for q_size data (Time)
fig5, ax7 = plt.subplots(figsize=(10, 5))

# Define colors for the lines and shaded regions
color3 = 'red'

# Plot q_size_t with times as x-axis
ax7.plot(times, q_size_t['mean'], marker='o', label='Time', color=color3)

# Fill the area around the line with a shadow representing errors
ax7.fill_between(times, 
                 [m - s for m, s in zip(q_size_t['mean'], q_size_t['std'])], 
                 [m + s for m, s in zip(q_size_t['mean'], q_size_t['std'])], 
                 color=color3, alpha=0.3)

# Set labels and title for the x-axis and y-axis
ax7.set_xlabel('Time', fontsize=12)
ax7.set_ylabel('Mean Queue Size (Time)', color=color3)
ax7.set_title('Mean Queue Size vs. Time')

# Display the legend for the Time data
lines6, labels6 = ax7.get_legend_handles_labels()
ax7.legend(lines6, labels6, loc='upper left')

# Customize the appearance of the Time plot
ax7.grid(True, linestyle='--', alpha=0.6)
ax7.spines['top'].set_visible(False)
ax7.spines['right'].set_visible(False)
ax7.tick_params(axis='both', which='both', length=6, width=1, labelsize=12)
